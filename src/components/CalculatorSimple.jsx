        ...
        {aberturaSlots.map((slot, idx) => (
          <g key={idx}>
            <rect
              ...
              fill={slot.type === 'puerta' ? '#FEE2E2' : '#DBEAFE'}
              stroke={slot.type === 'puerta' ? '#EF4444' : '#3B82F6'}
              strokeWidth="1.5"
              rx="3"
            />
            <foreignObject x={slot.x} y={slot.y} width={slot.w} height={slot.h}>
              <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {slot.type === 'puerta' ? (
                  <DoorIcon width={slot.w} height={slot.h} />
                ) : (
                  <WindowIcon width={slot.w} height={slot.h} />
                )}
              </div>
            </foreignObject>
          </g>
        ))}
        <text x={offsetX + wallWidth / 2} y={offsetY - 12} textAnchor="middle" className="text-[13px] font-semibold fill-[#031D5F]">
          {largo} m
        </text>
        <text x={offsetX - 18} y={offsetY + wallHeight / 2} textAnchor="middle" transform={`rotate(-90, ${offsetX - 18}, ${offsetY + wallHeight / 2})`} className="text-[13px] font-semibold fill-[#031D5F]">
          {alto} m
        </text>
      </svg>
    </div>
...
function CalculatingBar() {
  return (
    <div className="w-full bg-[#031D5F]/10 rounded-full h-2 overflow-hidden">
...
function InlineAbertura({ tipo, onAdd, isOpen, onToggle }) {
  ...
  return (
    <div className="mt-2 bg-[#031D5F]/5 rounded-2xl p-3 animate-fadeIn border border-[#0092FF]/20">
      ...
      <button onClick={handleAdd} className="w-full bg-[#0092FF] text-white py-2 rounded-xl text-xs font-bold btn-ripple shadow-md shadow-[#0092FF]/20">
        Agregar
      </button>
    </div>
  );
...
export default function CalculatorSimple({ recipe, onBack }) {
  ...
  const handleRecipeSelect = useCallback((rec, catalogData) => {
    ...
    setStep(0);
    setErrorMsg('');
    if (!rec || !Array.isArray(rec.productos)) return;
  ...
  const availablePanels = useCallback(() => {
    ...
    return (catalog || []).filter(p => selectedRecipe.productos.some(rp => rp.clave === p.clave) && p.tipo === 'panel');
  ...
  const getNumericParams = () => ({
    ...
    calibre: parseInt(params.calibre),
  ...
  const updateAbertura = (tipo, index, campo, valor) => {
    setParams(p => {
      const lista = tipo === 'puerta' ? [...(p.puertas || [])] : [...(p.ventanas || [])];
      if (lista[index]) lista[index] = { ...lista[index], [campo]: valor };
      return { ...p, [tipo === 'puerta' ? 'puertas' : 'ventanas']: lista };
    });
  };
  ...
  const handleToggleResistenteA = (checked) => {
    ...
    if (checked) setResistenteMsg('Cara A optimizada: SikaWall® sobre Nivel 1.');
  ...
  const handleToggleResistenteB = (checked) => {
    ...
    if (checked) setResistenteMsg('Cara B optimizada: SikaWall® sobre Nivel 1.');
    else setResistenteMsg('');
  };
  ...
  const handleCalculate = () => {
    ...
    setTimeout(() => {
      const materials = calculateFromRecipe(selectedRecipe, {
        ...numParams,
  ...
  const downloadPDF = async () => {
    ...
    a.href = url;
    a.download = 'cotizacion-obs.pdf';
    a.click();
    URL.revokeObjectURL(url);
  };

  // ---------- ENVÍO DIRECTO A WHATSAPP (SIEMPRE wa.me) ----------
  const shareText = () => {
    ...
    // Siempre abrimos el enlace de WhatsApp
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pb-24 pt-2" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      <button
        ...
        className="group mb-4 text-sm text-[#031D5F]/70 hover:text-[#0092FF] flex items-center gap-1.5 transition-all hover:bg-[#0092FF]/5 px-3 py-1.5 rounded-xl"
      >
            ...
            {step === 0 && (
              <div key="step0" className={direction === 'forward' ? 'animate-slide-in-right' : 'animate-slide-in-left'}>
                <div className="bg-white rounded-3xl card-depth p-6 border border-[#031D5F]/5">
                  ...
                  <p className="text-sm text-[#031D5F]/50 mb-6">Inserta las medidas de tu muro.</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <SegmentedGauge options={[{v:"0.305",label:"30.5cm"},{v:"0.406",label:"40.6cm"},{v:"0.61",label:"61cm"}]} value={params.separacionPostes} onChange={v => setParams({...params, separacionPostes: v})} />
                    </div>
                  </div>
                  ...
                  <div className="border-t border-[#031D5F]/10 pt-5 mb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <SegmentedGauge options={[{v:"26",label:"26 Std"},{v:"20",label:"20 Estruc"}]} value={params.calibre} onChange={v => setParams({...params, calibre: v})} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-[#031D5F]/10 pt-5 mb-6">
                    ...
                    {selectedRecipe.permitir_aislamiento && <ToggleSwitch checked={params.aislamiento} onChange={e => setParams({...params, aislamiento: e.target.checked})} label="Aislamiento" tooltip="Reduce ruido y temperatura al interior, recomendado para salas y dormitorios." />}
                    {selectedRecipe.permitir_refuerzos && <ToggleSwitch checked={params.refuerzos} onChange={e => setParams({...params, refuerzos: e.target.checked})} label="Refuerzos horizontales" tooltip="Añade refuerzos entre postes para mayor rigidez estructural." />}
                  </div>
                ...
                <div className="bg-white rounded-3xl card-depth p-6 border border-[#031D5F]/5">
                  ...
                  {params.factorCaras === '2' && (
                    <div className="flex border-b border-[#031D5F]/10 mb-4">
                      <button onClick={() => setActiveCaraTab('A')} className={`flex-1 py-2 text-xs font-bold border-b-2 ${activeCaraTab==='A'?'border-[#0092FF] text-[#0092FF]':'border-transparent text-[#031D5F]/40'}`}><Layers size={14}/> Cara A</button>
                      <button onClick={() => setActiveCaraTab('B')} className={`flex-1 py-2 text-xs font-bold border-b-2 ${activeCaraTab==='B'?'border-[#0092FF] text-[#0092FF]':'border-transparent text-[#031D5F]/40'}`}><Layers size={14}/> Cara B</button>
                    </div>
                  )}
                  ...
                  {availablePanels().length > 0 && (
                    <div className="bg-[#031D5F]/5 rounded-2xl p-4 border border-[#031D5F]/10 mb-4">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-xs font-extrabold text-[#031D5F] uppercase">Panel {activeCaraTab}</span>
                      </div>
                      ...
                      {selectedRecipe.permitir_resistente && (
                        <div className="border-t border-[#031D5F]/10 pt-3 mt-3">
                          <ToggleSwitch checked={activeCaraTab==='A'?params.incluirResistenteA:params.incluirResistenteB} onChange={e => activeCaraTab==='A'?handleToggleResistenteA(e.target.checked):handleToggleResistenteB(e.target.checked)} label="SikaWall®" tooltip="La tecnologia Estuka Sikawall le brinda a tu muro una alta resistencia a impacto y humedad." />
                          {(activeCaraTab==='A'?params.incluirResistenteA:params.incluirResistenteB) && (
                  ...
                  {selectedRecipe.permitir_secado_rapido && (params.selectedPanelA==='R804171'||params.selectedPanelB==='R804171') && !(params.incluirResistenteA&&params.incluirResistenteB) && (
                    <ToggleSwitch checked={params.secadoRapido} onChange={e => setParams({...params, secadoRapido: e.target.checked})} label="Secado rápido (45 min)" tooltip="El compuesto Easy set de secado rápido cuenta fraguado en minutos, recomendado en obras que requieren entregarse rapido." />
                  )}
              ...
              <div key="step2" className={direction === 'forward' ? 'animate-slide-in-right' : 'animate-slide-in-left'}>
                <div className="bg-white rounded-3xl card-depth p-6 border border-[#031D5F]/5">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {(Array.isArray(catalog)?catalog.filter(p=>p.tipo==='pintura'):[]).map(p => (
                                <button key={p.clave} onClick={() => setParams({...params, pinturaSeleccionada: p.clave})} className={`relative flex flex-col items-center p-3 rounded-xl border-2 transition-all ${params.pinturaSeleccionada===p.clave?'border-[#0092FF] bg-white shadow-md':'border-[#031D5F]/10 hover:border-[#0092FF]/30 bg-white/50'}`}>
                            ...
                            {params.pinturaSeleccionada && (
                              <div className="mt-2 bg-white rounded-lg p-2 text-xs text-[#031D5F]/70">
                                {(() => { const selected = catalog.find(p => p.clave === params.pinturaSeleccionada); return selected ? (selected.marca || selected.descripcion || selected.desc || selected.clave) : ''; })()}
                              </div>
                            )}
                          </div>
                          <div>
                            ...
                            <div className="animate-fadeIn">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {(Array.isArray(catalog)?catalog.filter(p=>p.tipo==='sellador'):[]).map(p => (
                                  <button key={p.clave} onClick={() => setParams({...params, selladorSeleccionado: p.clave})} className={`relative flex flex-col items-center p-3 rounded-xl border-2 transition-all ${params.selladorSeleccionado===p.clave?'border-[#0092FF] bg-white shadow-md':'border-[#031D5F]/10 hover:border-[#0092FF]/30 bg-white/50'}`}>
                                    {p.imagen ? <img src={p.imagen} alt="" className="w-12 h-12 object-cover rounded-lg mb-1" /> : <PaintBucket size={24} className="text-[#031D5F]/30" />}
                                    <span className="text-xs font-bold text-[#031D5F] text-center leading-tight">{p.descripcion||p.desc||p.clave}</span>
                              ...
                              {params.selladorSeleccionado && (
                                <div className="mt-2 bg-white rounded-lg p-2 text-xs text-[#031D5F]/70">
                                  {(() => { const selected = catalog.find(p => p.clave === params.selladorSeleccionado); return selected ? (selected.marca || selected.descripcion || selected.desc || selected.clave) : ''; })()}
                                </div>
                              )}
                            </div>
                          )}
                  ...
                  <div className="bg-[#031D5F]/5 rounded-2xl p-4 border border-[#031D5F]/10">
                    <ToggleSwitch checked={params.incluirSellado} onChange={e => setParams({...params, incluirSellado: e.target.checked})} label="Sello perimetral (SikaCryl®)" tooltip="Sella uniones y esquinas de tablaroca evitando grietas por vibración. Seca rápido, es lijable y garantiza un pintado impecable y sin marcas." />
